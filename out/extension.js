...
function activate(context) {
    ...
    context.subscriptions.push(vscode.languages.registerCompletionItemProvider('vox', { provideCompletionItems: buildCompletionItems }, '.', '(', ' '));
    // 6. Hover con documentación
    context.subscriptions.push(vscode.languages.registerHoverProvider('vox', {
...
function updateDiagnostics(document, collection) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        while ((m = tildeRx.exec(line)) !== null) {
            diagnostics.push(new vscode.Diagnostic(new vscode.Range(i, m.index, i, m.index + 7), 'Escribe "funcion" sin tilde — es una palabra reservada de Vox', vscode.DiagnosticSeverity.Error));
        }
        ...
        for (let c = 0; c < line.length; c++) {
            ...
            if (ch === '}') {
                braces--;
                if (braces < 0) {
                    diagnostics.push(new vscode.Diagnostic(new vscode.Range(i, c, i, c + 1), 'Llave "}" sin apertura correspondiente', vscode.DiagnosticSeverity.Error));
                    braces = 0;
                }
            }
            if (ch === '(') {
            ...
            if (ch === ')') {
                parens--;
                if (parens < 0) {
                    diagnostics.push(new vscode.Diagnostic(new vscode.Range(i, c, i, c + 1), 'Paréntesis ")" sin apertura correspondiente', vscode.DiagnosticSeverity.Error));
                    parens = 0;
                }
            }
        }
    ...
    if (braces > 0) {
        diagnostics.push(new vscode.Diagnostic(lastRange, `Faltan ${braces} llave(s) de cierre "}" en el documento`, vscode.DiagnosticSeverity.Error));
    }
    if (parens > 0) {
        diagnostics.push(new vscode.Diagnostic(lastRange, `Faltan ${parens} paréntesis de cierre ")" en el documento`, vscode.DiagnosticSeverity.Error));
    }
...
// ── Autocompletado ────────────────────────────────────────────────────────────
function buildCompletionItems(document) {
    const items = [];
...
function buildHover(document, position) {
    const range = document.getWordRangeAtPosition(position, /[a-zA-ZáéíóúñÁÉÍÓÚÑüÜ_][\wáéíóúñÁÉÍÓÚÑüÜ]*/);
    if (!range) {
        return undefined;
    }
